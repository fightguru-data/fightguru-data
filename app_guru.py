...
def load_data_v66():
 ...
 try:
  ...
  # Унификация имен и удаление лишних пробелов (Протокол №5)
  df['red_full_name'] = df['red_first_name'].fillna('').astype(str).str.strip() + " " + df['red_last_name'].fillna('').astype(str).str.strip()
  df['blue_full_name'] = df['blue_first_name'].fillna('').astype(str).str.strip() + " " + df['blue_last_name'].fillna('').astype(str).str.strip()
  
  df['date_start'] = pd.to_datetime(df['date_start'], errors='coerce')
  df['round_rank'] = df['round_code'].apply(lambda x: ROUND_MAP.get(str(x).upper(), (0, str(x)))[0])
  
  # Очистка предупреждений (Протокол №5)
  for side in ['red', 'blue']:
    # Проверяем все возможные варианты названий колонок
    col_names = [f'{side}_warning', f'{side}_warnings', f'{side}_total_warnings']
    found_col = next((c for c in col_names if c in df.columns), None)
    if found_col:
      df[f'{side}_warn_clean'] = pd.to_numeric(df[found_col], errors='coerce').fillna(0).astype(int)
    else:
      df[f'{side}_warn_clean'] = 0
...
if "bot_active" not in st.session_state:
 try:
   ...
   def run_bot_service():
    ...
    def welcome(m): bot.reply_to(m, "FIGHTGURU: Введите фамилию.")
    @bot.message_handler(func=lambda m: True)
    def bot_search(m):
      q = m.text.strip().lower()
      if len(q) < 3: return
      if df is not None:
        b_res = df[(df['red_last_name'].str.lower().str.contains(q, na=False)) | (df['blue_last_name'].str.lower().str.contains(q, na=False))].copy()
        if not b_res.empty:
          b_res = b_res.sort_values('date_start', ascending=False).head(5)
          msg = f"📊 ДОСЬЕ: {q.upper()}\n"
          for _, r in b_res.iterrows():
            msg += f"\n🗓 {r['date_start'].year} | {r['tournament_name'][:30]}\nСчет: {int(r.get('red_score',0))}:{int(r.get('blue_score',0))}\n"
 ...
 if nav == "👤 Досье":
  ...
  if search_input:
   ...
   if not matches.empty:
    ...
    for _, r in matches.iterrows():
     if search_low in str(r['red_last_name']).lower():
      ...
      if c_dob and pd.notna(r[c_dob]): 
          dob_val = str(r[c_dob]).strip()
          if dob_val.lower() != 'nan': dob_list.append(dob_val)
    ...
    # Берем наиболее часто встречающуюся строку DOB (чтобы исключить ошибки в одной из строк базы)
    ...
    for _, row in matches.iterrows():
     ...
     # --- ПРЕДУПРЕЖДЕНИЯ (Использование очищенных данных) ---
     my_w = row['red_warn_clean'] if is_red else row['blue_warn_clean']
     op_w = row['blue_warn_clean'] if is_red else row['red_warn_clean']
  ...
  if not fin_matches.empty:
   def get_w_info(r):
    if str(r['winner_athlete_id']) == str(r['red_id']): return r['red_full_name'], str(r['red_nationality_code']).upper()
